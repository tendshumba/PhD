...
for x in evals do 
  E:=Eigenspace(M,x);
  Bas:=[[&+[(E.i)[j]*BB[j]:j in [1..780]]:i in [1..Dimension(E)]];
  if forall{v:v in Bas|v^p eq v} then /*everything in the space is even*/
   ...
   elif forall{v:v in Bas|v^p eq -v}  then /*everything is odd.*/ 
	...
	for i in [1..#Inds[s]] do 
	  for j in [1..#Inds[t]] do 
	    if i le j then 
	      ...
	      pd:=scaledProd1(u,v);
	      for l in [1..#Inds] do 
		if &+[pd[k]*ProjSpaces[l][k]:k in [1..Degree(pd)]] ne Zero(V) then
